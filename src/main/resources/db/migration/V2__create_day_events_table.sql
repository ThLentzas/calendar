...
CREATE TABLE IF NOT EXISTS day_events (
    ...
    CONSTRAINT fk_day_events_users_id FOREIGN KEY (organizer_id) REFERENCES users (id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS day_event_slots(
    ...
    CONSTRAINT fk_day_event_slots_day_events_id FOREIGN KEY (event_id) REFERENCES day_events ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS day_event_slot_guest_emails (
    event_slot_id uuid,
    email TEXT,
    CONSTRAINT pk_day_event_slot_guest_emails PRIMARY KEY (event_slot_id, email),
    CONSTRAINT fk_day_event_slot_guest_emails_day_event_slots_id FOREIGN key (event_slot_id) REFERENCES day_event_slots(id) ON DELETE CASCADE
);