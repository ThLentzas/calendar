...
public final class TimeEventUpdateRequestValidator implements ConstraintValidator<ValidTimeEventUpdateRequest, TimeEventRequest> {
    ...
    public boolean isValid(TimeEventRequest eventRequest, ConstraintValidatorContext context) {
        if (EventUtils.hasEmptyEventUpdateRequestProperties(eventRequest)) {
        ...
        return EventUtils.hasRequiredDateTimeProperties(eventRequest.getStartTime(), eventRequest.getEndTime(), eventRequest.getStartTimeZoneId(), eventRequest.getEndTimeZoneId(), context)
                && EventUtils.hasValidEventRequestProperties(eventRequest, context);
    }
}
