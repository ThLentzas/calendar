...
import org.example.google_calendar_clone.user.contact.ContactRequestService;
...
public class UserService {
     ...
     */
    @Transactional
    void addContact(CreateContactRequest contactRequest, Jwt jwt) {
        ...
        this.contactRequestService.addContact(sender, receiver);
    }

    /*
        The current user(receiver) received a contact request and either accepts or rejects the request
     */
    void updateContact(UpdateContactRequest contactRequest, Jwt jwt) {
        Long receiverId = Long.valueOf(jwt.getSubject());
        Long senderId = contactRequest.senderId();
        this.contactRequestService.updateContact(senderId, receiverId, contactRequest.action());
    }
    ...
    List<PendingContactRequest> findPendingContactsRequests(Jwt jwt) {
        ...
        return contactRequests.stream()
                .map(contactRequest -> new PendingContactRequest(
                        this.profileConverter.convert(contactRequest.getSender()),
