...
@Setter
@SuperBuilder
@ValidCreateTimeEventRequest
@EqualsAndHashCode(callSuper = true)
public class CreateTimeEventRequest extends AbstractEventRequest {
    @NotNull(message = "The start time of the event is required. Please provide one")
    // null will return true
    private LocalDateTime startTime;
    @NotNull(message = "The end time of the event is required. Please provide one")
    private LocalDateTime endTime;
    // It throws ZoneRulesException: Unknown time-zone ID, for invalid timezone during deserialization
    @NotNull(message = "The time zone for the event's start time is required. Please provide one")
    private ZoneId startTimeZoneId;
    @NotNull(message = "The time zone for the event's end time is required. Please provide one")
